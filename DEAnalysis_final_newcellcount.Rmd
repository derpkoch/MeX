...
```{r}
countData <- read_excel("../Final/TableS1.xlsx",
                        sheet = "RAW") %>%
...
ggplot(pc.top,
       aes(x = parse_number(as.character(timepoint)), y = count, color = batch)) + 
  geom_point() + geom_smooth(se = FALSE, method = "loess") +
  scale_y_log10() +
  theme_linedraw()
```

Fit DESeq model. Data from all time points and replicates are used to estimate dispersion, but only the contrast $t_3$ vs $t_0$ will be computed and tested in the end.
...
Since we are only interested in peptides which may be toxic, i.e. those whose log fold change at $t_3$ and $t_1$ vs $t_0$ is negative, we set `altHypothesis = "less"`.
...
#calculation of CV
test = DESeq2_database %>% 
  mutate(CV1 = lfcShrink1SE/abs(lfcShrink1), CV2 = lfcShrink2SE/abs(lfcShrink2),CV3 = lfcShrink3SE/abs(lfcShrink3), CV = (CV1+CV2+CV3)/3) 
```

## Optical density

Compute mean optical densities, $\bar{\operatorname{OD}}(t)$, across time points. Divide each mean OD by that at $t_0$, and take log2.
...
Compute the the optical density of each peptide-expressing strain in Mex
...
Convert the data containing DESeq2 result of $t_3$ vs $t_0$, p-values, shrunken LFCs (vs $t_0$) and their SEs, as well as the number of cells at each time point relative to $t_0$. frame into a tibble and add information from the dictionary, such as DNA and AA sequence.

```{r}
dictionary <- read.csv("../Excel lists/ID_DNA_AA.csv")
DESeq2_database <- res.merged %>%
  as.data.frame() %>%
...
#saveRDS(database, "../Final/DESeq2_database_1120.rds")
```
